...
function Tune(overrides)
    ...
    local wilson_attack = 34 * multiplayer_attack_modifier--威尔逊的基础攻击
    local wilson_health = 150--威尔逊的基础生命
    local calories_per_day = 75--每天掉的饥饿值

    local wilson_attack_period = .1--威尔逊的攻击间隔
    ...
    ----------------穿装备以后的行走速度--------------------
        CANE_SPEED_MULT = 1.25,--拿手杖后行走速度1.25倍
        PIGGYBACK_SPEED_MULT = 0.9,--猪皮包速度变0.9
        HEAVY_SPEED_MULT = .15,
	...
	---------------------动物的一些逻辑设定--------------------
        CRITTER_HUNGERTIME = total_day_time * 3.25,--饥饿时间
        ...
        CRITTER_EMOTE_DELAY = seg_time,--做表情间隔
        CRITTER_NUZZLE_DELAY = total_day_time * 0.5,--亲昵间隔
        CRITTER_PLAYFUL_DELAY = seg_time * 0.4,--调皮间隔
        CRITTER_COMBAT_LOOP_CHANCE = 0.33,--互相战斗概率
        CRITTER_WANTS_TO_BE_PET_TIME = seg_time * 0.5,--想要成为宠物
	...
	--------------------牛相关属性和逻辑-----------------------
        ...
        BEEFALO_HAIR_GROWTH_DAYS = 3,--牛毛生长速度
	...
	-----------------------------------------------------
        PROTOTYPER_TREES =
        {
            SCIENCEMACHINE = TechTree.Create({--科学机器
                SCIENCE = 1,
                MAGIC = 1,
            }),

            ALCHEMYMACHINE = TechTree.Create({--炼金机器
                SCIENCE = 2,
                MAGIC = 1,
            }),
            ...
            ANCIENTALTAR_LOW = TechTree.Create({--祭坛
                ANCIENT = 2,
            }),
	...
	-----------------------------------------------------
        ...
        POOP_FERTILIZE = day_time,--便便
        POOP_SOILCYCLES = 10,
        POOP_WITHEREDCYCLES = 1,
        POOP_CAN_USES = 8,
        GUANO_FERTILIZE = day_time * 1.5,--鸟便便
        GUANO_SOILCYCLES = 12,
        GUANO_WITHEREDCYCLES = 1,

        GLOMMERFUEL_FERTILIZE = day_time,--格罗门燃料
        GLOMMERFUEL_SOILCYCLES = 8,

        SPOILEDFOOD_FERTILIZE = day_time/4,--坏掉的食物
	...
	---------------------------------------------------------
        ...
        CATCOONHAT_PERISHTIME = total_day_time*10,--浣熊帽
        GOGGLES_PERISHTIME = total_day_time*10,
		
		
        ------------------植物生长时间-------------------
		-------------------------------------------------
        GRASS_REGROW_TIME = total_day_time*3,--草三天一次
        SAPLING_REGROW_TIME = total_day_time*4,--树杈四天一次
        MARSHBUSH_REGROW_TIME = total_day_time*4,--尖刺灌木
        ...
        CROW_LEAVINGS_CHANCE = .3333,--乌鸦掉毛概率
	...
	---------冰箱的保鲜-----------        
		PERISH_FRIDGE_MULT = .5,--变质速度变为0.5
	...
	----------一些材料的参数（？）-----------
        REPAIR_CUTSTONE_HEALTH = 50,--石砖
        REPAIR_ROCKS_HEALTH = 50/3,--石头
        REPAIR_GEMS_WORK = 1,--宝石
        REPAIR_GEARS_WORK = 1,--齿轮

        REPAIR_THULECITE_WORK = 1.5,--铥矿石
        REPAIR_THULECITE_HEALTH = 100,

        REPAIR_THULECITE_PIECES_WORK = 1.5/6,
        REPAIR_THULECITE_PIECES_HEALTH = 100/6,

        REPAIR_BOARDS_HEALTH = 25,--木板
        REPAIR_LOGS_HEALTH = 25/4,
        REPAIR_STICK_HEALTH = 13,
        REPAIR_CUTGRASS_HEALTH = 13,--草
	...
	-------------主角的掉脑残逻辑-------------------
        SANITY_DAY_GAIN = 0,--100/(day_time*32),--白天

        SANITY_NIGHT_LIGHT = -100/(night_time*20),--晚上很亮
        SANITY_NIGHT_MID = -100/(night_time*20),--晚上中等亮度
        SANITY_NIGHT_DARK = -100/(night_time*2),--全黑
